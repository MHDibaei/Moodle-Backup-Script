...
if [ -f "$LATEST_MOODLE_DIR" ]; then
    log_message "Extracting Moodle directory: $LATEST_MOODLE_DIR to $MOODLE_ROOT_DIR"
    tar -xzf "$LATEST_MOODLE_DIR" -C "/"
else
...
if [ -f "$LATEST_MOODLEDATA_DIR" ]; then
    log_message "Extracting Moodle directory: $LATEST_MOODLEDATA_DIR to $MOODLE_ROOT_DIR"
    tar -xzf "$LATEST_MOODLEDATA_DIR" -C "/"
else
    log_message "Unable to find the latest Moodle directory archive."
    exit 1
fi
...
log_message "Moodle restoring process done!"
